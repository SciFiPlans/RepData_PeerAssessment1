...
```{r}
library(lattice)
dayt.f <-factor(actdata2$daytype,levels=c("weekday","weekend"), labels=c("Weekday","Weekend"))
#xyplot(actdata2$steps~actdata2$interval|dayt.f,
xyplot(actdata2$steps~actdata2$interval|actdata2$daytype,
   main="Density Plot by Number of Cylinders",
   xlab="Miles per Gallon",type = "l",layout=c(1,2))
```

```{r}
df <- data.frame(m = numeric(),  i = as.Date(character()),   dt = character())
for (Date in levels(actdata2$date)) { 
  MeanSteps <- mean(actdata2$steps[actdata2$date == Date])
  DayType <- actdata2$daytype[actdata2$date == Date][1]
  df <- rbind(df,data.frame(MeanSteps,Date,DayType))
}
```

```{r}
intervals <- actdata$interval[actdata$date == "2012-11-04"]
ms2 <- tapply(actdata2$steps[actdata2$daytype == "weekday"],actdata2$interval[actdata2$daytype == "weekday"],FUN = mean)
ms3 <- tapply(actdata2$steps[actdata2$daytype == "weekend"],actdata2$interval[actdata2$daytype == "weekend"],FUN = mean)
d0 <- cbind(ms2,ms3,intervals)
layout(c(1,2))
plot(intervals,ms2,type = "l",main = "weekday",ylab = "steps",xlab = "")
plot(intervals,ms3,type = "l",main = "weekend",ylab = "steps")
```


```{r}
#by(actdata$steps,actdata$interval,sum)
#ugh <- tapply(actdata$steps,actdata$interval,FUN = mean)
#summary(cars)
```
